...
# Parse Settings.inc
# ------------------------------------------------------------------
$Token = ''; $Repo1 = ''; $Repo2 = ''; $Repo3 = ''; $AutoRefreshMin = 0; $Theme = 'Green'; $Opacity = 220
...
if ($Token -eq '' -or $Token -match '^ghp_x+$' -or $Token -eq 'ghp_your_token_here') {
    ...
    foreach ($s in @(
        '; CommitView - Error state', '[Rainmeter]', 'Update=5000', 'AccurateText=1', '',
        ...
        'X=40', 'Y=52', 'W=20', 'H=20',
        'LeftMouseUpAction=["wscript.exe" "#CURRENTPATH#launcher_commits.vbs"]', 'ToolTipText=Click to reload', ''
    )) { $eLines.Add($s) }
    ...
    if (Test-Path $rm2) { Start-Process $rm2 -ArgumentList "!Refresh `"$cfg2`" `"CommitView.ini`"" -ErrorAction SilentlyContinue }
    exit 1
}
...
foreach ($repo in $configuredRepos) {
    ...
    try {
        ...
        foreach ($c in $commits) {
            $msg    = ($c.commit.message -split "`n")[0].Trim() -replace '#', '＃' -replace '=', '＝' -replace '\[', '［' -replace '\]', '］'
